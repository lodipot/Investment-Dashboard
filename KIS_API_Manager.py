...
# [API 변경] 결제내역(CTOS4001R) 대신 체결내역(TTTS3012R) 사용
# URL: /uapi/overseas-stock/v1/trading/inquire-ccld (기간별 체결 조회 가능)
def get_trade_history(start_date, end_date):
    ...
    # [수정된 URL] inquire-period-ccld (X) -> inquire-ccld (O)
    res = _request_api('GET', f"{URL_BASE}/uapi/overseas-stock/v1/trading/inquire-ccld", headers, params=params)
            ...
            for item in data['output1']:
                ...
                if qty > 0:
                    mapped_item = {
                        ...
                        'ccld_qty': str(qty),
                        # 체결단가 우선순위: avg_prvs(체결평균) > ft_ccld_unpr3(체결가)
